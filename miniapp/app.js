...
async function init() {
  ...
  if (!currentUser && debugId) {
    currentUser = { id: debugId, first_name: 'DebugUser', username: 'debuguser' };
  ...
  if (!currentUser) {
    document.getElementById('loading').innerHTML = `❌ Buka MiniApp dari dalam Telegram Asli!<div style="margin-top:20px;font-size:0.9rem;color:#888;">Simulasi PC: <code>?debugId=TELEGRAM_ID</code></div>`;
  }
   // Load User Data
  try {
...
async function loadLeaderboard(filter) {
    ...
    try {
        ...
        if (!data.leaderboard || data.leaderboard.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding:20px;">Belum ada data klasemen.</div>';
            return;
        }
        ...
        container.innerHTML = '<div style="text-align:center; color:#ef4444;">Gagal memuat klasemen.</div>';
    }
}
